...
test("Renders the EditForm when editing = true and does not render EditForm when editing = false", async () => {
    const { rerender } = render(<ColorList colors={testColorList} toggleEdit={true} />);
    await waitFor(() => {
        ...
        rerender(<ColorList colors={testColorList} toggleEdit={false} />);
        const checkEditMenu = screen.queryAllByTestId('editMenu');
        expect(checkEditMenu).toHaveLength(1);
    })
});